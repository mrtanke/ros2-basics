...
def generate_launch_description():
    ...
    action_declare_arg_mode_path = launch.actions.DeclareLaunchArgument(
        name='model', default_value=str(default_urdf_path), description='Loaded model file path'
    )
    ...
    # 4. create robot_state_publisher and joint_state_publisher nodes
    ...
    return launch.LaunchDescription([
        ...
        action_joint_state_publisher, # publish joint states 
        action_robot_state_publisher, # publish robot state (URDF model)
        action_rviz_node # visualize in rviz
    ])